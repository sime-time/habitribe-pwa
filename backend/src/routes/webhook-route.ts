...
router.post("/waitlist", async (c: Context) => {
	try {
		...
		if (!mailerliteApiKey || !mailerliteGroupId) {
			console.error(
				"MailerLite API Key or Group ID is missing from environment variables.",
			);
		...
		const instagram = submission["What is your instagram?"];
		const name = submission["What is your name?"];
		const clients = submission["How many clients do you currently coach?"];
		const challenge =
			submission[
				"What's your biggest challenge with tracking client progress?"
			];

		// If the email is not present, return an error.
		if (!email) {
			return c.json(
				{ error: "Email address not found in the form submission." },
				400,
			);
		}
		...
		if (mailerliteResponse.ok) {
			...
			console.error(
				`MailerLite API error: ${mailerliteResponse.status} - ${errorText}`,
			);
